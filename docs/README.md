# Sam's Secutiy Vault
CLI credential manager with AES-256 Encryptio

# Overview
A Python-based CLI tool for DevSecOps enthusiasts to securely manage password, API keys, and tokens with a high level enccryption 

# Key Features

- **High-Level Encryption**: AES-256-GCM with authenticated Encryption
- **Secure key derivation**: PBKDF2 with 100,000+ iterations
- **Memory Safety**: No plaintext credentials ever written to disk 
- **Clean CLI**: Rick terminal interface with user validation

# Developement Status

- **Phase 1**: Core cryptopgraphic implementation and code. (In Progress)
- **Phase 2**: CLI interface with Click framework
- **Phase 3**: Advanced features (backups, import/export, MFA) 
- **Phase 4**: Comprehensive testing 


